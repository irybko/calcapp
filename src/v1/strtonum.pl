...
strtonum(NumStrExpr, Number) :-
	...
	(
		First == 45 ->	
			select(First, SCodes, RemList),
			codestonum(RemList, Step),
			Number is 0 - Step
		;
...
codestonum(StrCodes, Num) :-
	...
	(
		member(DotCode, StrCodes) -> 	
			...
			getwholenumber(WCodes, WNum),
			getfractnumber(FCodes, FNum),
...
getwholenumber(CharCodes, WN) :-
	charstodigits(CharCodes, WDigitCodes),
	digtonumlist(WDigitCodes, NumList),
	listsumm(NumList, WN).
...
getfractnumber(CharCodes, FN) :-
	listlen(CharCodes, Sz),
	getwholenumber(CharCodes, WNum),
	digtonum(WNum, -Sz, FN).
...
digtonum(Digit, Power, Num) :-
	Num is Digit * 10 ** Power.
